    ...
    <Box className="HomeContainer">
      ...
      <AppBar className="AppBar" position="fixed" open={drawerOpen}>
        <Toolbar>
          ...
          <Box className="Profile" sx={{mr: 2}}>
              <Avatar alt="SAI GANESH" src={myImage} sx={{cursor: 'pointer', border: '3px solid white'}} onClick={handleClick}/>
              <Menu
                id="basic-menu"
                anchorEl={anchorEl}
                open={openAvatarMenu}
      ...
      <Drawer variant="permanent" open={drawerOpen}>
        <DrawerHeader className={`${drawerOpen ? "show sideFloatClass": "hide"}`} >
          <Avatar alt={myImage} src={myImage} sx={{cursor: 'pointer', margin: 'auto'}} />
